class Board
  ...
  attr_reader :board
  attr_reader :size
  def initialize(player1, player2, size = 1)
    @player1 = player1
    @player2 = player2
    @size = size
  ...
  def check_winner
    ...
    return nil if @board.none? { |i| i.nil? }
    false
  end
  ...
  def check_diag
    ...
    return true if !@board[@size - 1].nil? && (0..(@size - 1)).all? { |x| @board[x * @size + (@size - x - 1)] == @board[@size - 1] }
      false
  end
